!(function() {
  ...
  class i {
    ...
    handleRequest({ request: e, event: t }) {
      ...
      let o,
        i = r && r.handler;
      if ((!i && this._defaultHandler && (i = this._defaultHandler), i)) {
        try {
    ...
    findMatchingRoute({ url: e, request: t, event: s }) {
      ...
      for (const r of n) {
        let n;
        const o = r.match({ url: e, request: t, event: s });
        if (o)
          return (
            (n = o),
  ...
  let a;
  const c = () => (
    a || ((a = new i()), a.addFetchListener(), a.addCacheListener()), a
  );
  function l(e, s, o) {
    let i;
    if ("string" == typeof e) {
      const t = new URL(e, location.href);
      i = new n(({ url: e }) => e.href === t.href, s, o);
    } else if (e instanceof RegExp) i = new r(e, s, o);
    else if ("function" == typeof e) i = new n(e, s, o);
    else {
      if (!(e instanceof n))
  ...
  const u = {
    ...
    h = (e) =>
      [u.prefix, e, u.suffix].filter((e) => e && e.length > 0).join("-"),
    d = (e) => e || h(u.runtime),
    p = new Set();
  const f = (e, t) => e.filter((e) => t in e),
    g = async ({ request: e, mode: t, plugins: s = [] }) => {
      const n = f(s, "cacheKeyWillBeUsed");
      let r = e;
      for (const e of n)
    ...
    m = async ({
      ...
      const o = await self.caches.open(e),
        i = await g({ plugins: r, request: t, mode: "read" });
      let a = await o.match(i, n);
      for (const t of r)
    ...
    w = async ({
      ...
      const c = await g({ plugins: i, request: s, mode: "write" });
      if (!n) throw new t("cache-put-with-no-response", { url: o(c.url) });
      const l = await (async ({
        request: e,
        response: t,
        event: s,
        ...
        for (const t of n)
          if ("cacheWillUpdate" in t) {
            ...
            if (
              ((r = await n.call(t, { request: e, response: r, event: s })), !r)
            )
              break;
          }
        return o || (r = r && 200 === r.status ? r : void 0), r || null;
      })({ event: r, plugins: i, response: n, request: c });
      if (!l) return;
      const u = await self.caches.open(e),
        h = f(i, "cacheDidUpdate"),
        d =
          h.length > 0
            ? await m({ cacheName: e, matchOptions: a, request: c })
            : null;
      try {
        await u.put(c, l);
      } catch (e) {
    ...
    y = m,
    v = async ({ request: e, fetchOptions: s, event: n, plugins: r = [] }) => {
      if (
  ...
  class b {
    ...
    async getAllMatching(
      ...
      return await this.transaction([e], "readonly", (i, a) => {
        const c = i.objectStore(e),
          l = t ? c.index(t) : c,
          u = [],
          h = l.openCursor(s, n);
        h.onsuccess = () => {
    ...
    async transaction(e, t, s) {
      return (
        ...
        await new Promise((n, r) => {
          const o = this._db.transaction(e, t);
          (o.onabort = () => r(o.error)),
            (o.oncomplete = () => n()),
            s(o, (e) => n(e));
        })
    ...
    async _call(e, t, s, ...n) {
      return await this.transaction([t], s, (s, r) => {
  ...
  for (const [e, t] of Object.entries(R))
    for (const s of t)
      s in IDBObjectStore.prototype &&
        (b.prototype[s] = async function(t, ...n) {
  ...
  var x = new class {
    constructor(e = {}) {
      if (((this._cacheName = d(e.cacheName)), e.plugins)) {
        const t = e.plugins.some((e) => !!e.cacheWillUpdate);
        this._plugins = t ? e.plugins : [q, ...e.plugins];
      } else this._plugins = [q];
    ...
    async handle({ event: e, request: s }) {
      ...
      if ((a || (a = await i), !a)) throw new t("no-response", { url: s.url });
      return a;
    }
    _getTimeoutPromise({ request: e, logs: t, event: s }) {
      ...
      return {
        promise: new Promise((t) => {
          n = setTimeout(async () => {
            t(await this._respondFromCache({ request: e, event: s }));
    ...
    async _getNetworkPromise({ timeoutId: e, request: t, logs: s, event: n }) {
      ...
      if ((e && clearTimeout(e), r || !o))
        o = await this._respondFromCache({ request: t, event: n });
      else {
        const e = o.clone(),
          s = w({
  ...
  l(function(e) {
    var t = e.url;
    return (
      "/assets/js/service-worker.js" !== t.pathname &&
      "/service-worker.js" !== t.pathname &&
      "/" === t.pathname
    );
  }, x),
    l(function(e) {
      ...
      return (
        "/assets/js/service-worker.js" !== t.pathname &&
        "/service-worker.js" !== t.pathname &&
        ("string" == typeof /\.(js|css|png)$/
          ? t.pathname === /\.(js|css|png)$/
          : /\.(js|css|png)$/ instanceof RegExp &&
            /\.(js|css|png)$/.test(t.pathname))
      );
    }, x);
})();
